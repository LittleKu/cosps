...
private:
	...
	void CalcLayoutInfo(HWND hWnd, CSize sizeTypeTL, CSize sizeTypeBR, CResizableLayout::LayoutInfo &layout);
protected:
	...
	//Update the anchor's current layout info
	BOOL RefreshAnchor(HWND hWnd);
	
	//Update all the anchor's current layout info
	BOOL RefreshAllAnchors();
