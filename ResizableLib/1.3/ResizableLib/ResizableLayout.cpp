...
void CResizableLayout::CalcLayoutInfo(HWND hWnd, CSize sizeTypeTL, CSize sizeTypeBR, CResizableLayout::LayoutInfo &layout)
{
	...
	layout.hWnd = hWnd;
	layout.sizeTypeTL = sizeTypeTL;
	layout.sizeTypeBR = sizeTypeBR;
	layout.sizeMarginTL = sizeMarginTL;
	layout.sizeMarginBR = sizeMarginBR;
	layout.sWndClass = sClassName;
...
{
	LayoutInfo layout;
	CalcLayoutInfo(hWnd, sizeTypeTL, sizeTypeBR, layout);
...
BOOL CResizableLayout::RefreshAnchor(HWND hWnd)
{
	POSITION pos;
	...
	CalcLayoutInfo(layout.hWnd, layout.sizeTypeTL, layout.sizeTypeBR, layout);

	//Refresh the original layout info
	m_listLayout.SetAt(pos, layout);

	return TRUE;
}

BOOL CResizableLayout::RefreshAllAnchors()
{
	POSITION pos = m_listLayout.GetHeadPosition();
	while (pos != NULL)
	{
		// get layout info
		//This function get the reference of the 
