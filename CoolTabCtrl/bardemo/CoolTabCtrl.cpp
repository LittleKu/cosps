...
void CCoolTabCtrl::OnPaint() 
{
	...
	if(m_pBkgBitmap != NULL)
	{
		...
		memDC.SelectObject(m_pBkgBitmap);
...
void CCoolTabCtrl::GetTabsRect(LPRECT lpRect)
{
	CWnd::GetClientRect(lpRect);
	if(m_nStyle&TCS_DOWN)
	{
		lpRect->top = lpRect->bottom - ITEMBUTTON_HEIGHT;
	}
	else if(m_nStyle&TCS_UP)
	{
		lpRect->bottom = lpRect->top + ITEMBUTTON_HEIGHT;
	}
}
...
void CCoolTabCtrl::CPageItem::Draw(CDC *pDC, UINT nStyle, BOOL bActive, BOOL bHovered)
{
	CRect rect = m_rect;
	rect.top += 5;
	/*
